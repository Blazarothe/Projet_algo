...
void nouvelle_partie(int *nmbre_joueurs,int matrix[49][3][3]) {
    ...
    int i, action, deplacement, choix;
    int num_pion[4];
   ...
   int player=4;
    ...
    {
        depla(matrix,player,compte_T,compte_L,compte_I,compteL_tresors);
        for (i = 0; i < *nmbre_joueurs; i++)
        {
            ...
            if (action == 1)
            {
                do
                {
                    ...
                    depla(matrix,player,compte_T,compte_L,compte_I,compteL_tresors); //call la fonction pour que le joeuur se deplace
