...
void nouvelle_partie(int *nmbre_joueurs,int matrix[49][3][3]) {
    ...
    if (*nmbre_joueurs < nombre_joueurs_min ||
        *nmbre_joueurs > nombre_joueurs_max) //Blindage nombre de joueurs, boucle infinie si input != nombre
    {
        do {
            fflush(stdin);
            printf("\n                            [L]  [e]    [L]  [a]  [b]  [y]  [r]  [i]  [n]  [t]  [h]  [e]\n\n\n");
        ...
        if (num_pion[i] == num_pion[i - 1] || num_pion[i] == num_pion[i - 2] || num_pion[i] == num_pion[i - 3]) {
            do {
                ...
                printf("Joueur %i : Quel pion choisissez-vous (Repondez par un numero pas encore pris par votre camarade) ?\n",
                       i + 1);
                scanf("%i", &num_pion[i]);
            } while (num_pion[i] == num_pion[i - 1] || num_pion[i] == num_pion[i - 2] ||
                     num_pion[i] == num_pion[i - 3]);
        }
        printf("*  Comment se nommera votre personnage ?\n"); //Definition du nom du personnage
        scanf("%s", &nom_personnage[i][le_prenom_devient_long]);
    }
    resume_pions(*nmbre_joueurs, num_pion,
                 nom_personnage); //Sous-programme faisant apparaitre un resume des personnages
    color(couleur_blanche, 0);
    ...
    zebi(matrix, *nmbre_joueurs, compte_T, compte_L, compte_I, compteL_tresors);
    while (nmbre_joueurs != 0) {
        //if(player == 2)
        //    player = 3;
        //else
        //    player = 2;
        depla(matrix, player, compte_T, compte_L, compte_I, compteL_tresors, deplacement,nmbre_joueurs);
        for (i = 0; i < *nmbre_joueurs; i++) {
            if(player == 2)
                player = 3;
            else
            ...
            if (action == 1) {
                ...
                do {
                    printf("* Appuyez sur Z pour aller tout droit\n"
                           ...
                           "appuyer sur e pour mettre fin a votre tour\n");
                } while (depla(matrix, player, compte_T, compte_L, compte_I, compteL_tresors,deplacement,nmbre_joueurs) == 1);
            }
            if (action == 2) {
                system("cls");
                choix = menu();
